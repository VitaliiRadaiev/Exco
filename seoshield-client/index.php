...
if (isset($_GET['check_tracks']) && isset($_GET['action'])){
    ...
    if (!isset($_GET['type'])){
        if ($_GET['action'] == 'count'){
            ...
            fputcsv($output, array('URL', 'CATEGORY_NAME', 'FILTER_1', 'FILTER_2', 'FILTER_3', 'FILTER_4', 'TITLE', 'H1', 'DESCRIPTION', 'BREADCRUMBS', 'ROBOTS', 'SEO_TEXT', 'PRODUCTS_NUM', 'VISITS', 'DATE_ADD',));
            $q = $db->mysql_query("SELECT `url`, `category_name`, `filter_1`, `filter_2`, `filter_3`, `filter_4`, `title`, `h1`, `description`, `breadcrumbs`, `robots`, `seo_text`, `products_num`, `visits`, `date_add` FROM ".$GLOBALS['SEOSHIELD_CONFIG']['track_visits']['table_name']);
        ...
        if ($_GET['action'] == 'count'){
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_info_name'])){
                $q = $db->mysql_query("SELECT `id` FROM ".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_name']);
                echo $db->mysql_num_rows($q);
            }
            exit();
        } elseif ($_GET['action'] == 'download'){
            header('Content-Type: text/csv; charset=utf-8');
            header('Content-Disposition: attachment; filename=export_product_visits_'.$_SERVER["HTTP_HOST"].'_'.date('d.m.Y_H.i.s', time()).'.csv');
            ...
            $q = $db->mysql_query("SELECT `url`, `h1`, `category_name`, `parent_category_name`, `breadcrumbs`, `product_manufacturer`, `product_model`, `product_code`, `short_desc`, `full_desc`, `reviews_num`, `price`, `discount_price`, `availability`, `description`, `robots`, `seo_text`, `visits`, `date_add`, `name`, `value`
                FROM `".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_name']."`
                RIGHT JOIN `".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_info_name']."` on `".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_info_name']."`.`product_id` = `".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_name']."`.`product_id`");
            $rows = array();
            while ($row = $db->mysql_fetch_assoc($q)) {
                $rows[] = $row;
                // fputcsv($output, $row);
            ...
            $custom_head = [];
            foreach($rows as $row){
                if (!isset($products_info[$row['url']])){
                    $products_info[$row['url']] = [
                        'url' => $row['url'], 'h1' => $row['h1'], 'category_name' => $row['category_name'], 'parent_category_name' => $row['parent_category_name'], 'breadcrumbs' => $row['breadcrumbs'], 'product_manufacturer' => $row['product_manufacturer'], 'product_model' => $row['product_model'], 'product_code' => $row['product_code'], 'short_desc' => $row['short_desc'], 'full_desc' => $row['full_desc'], 'reviews_num' => $row['reviews_num'], 'price' => $row['price'], 'discount_price' => $row['discount_price'], 'availability' => $row['availability'], 'description' => $row['description'], 'robots' => $row['robots'], 'seo_text' => $row['seo_text'], 'visits' => $row['visits'], 'date_add' => $row['date_add']
                    ];
                }
                if (!isset($products_info[$row['url']]['attrs'][$row['name']])){
                    if (!in_array($row['name'], $custom_head)){
                        $custom_head[] = $row['name'];
                    }
                    $products_info[$row['url']]['attrs'][$row['name']] = [];
                }
                $products_info[$row['url']]['attrs'][$row['name']][] = html_entity_decode($row['value']);
            }

            $head = array_merge(['URL', 'H1', 'CATEGORY NAME', 'PARENT CATEGORY NAME', 'BREADCRUMBS', 'PRODUCT MANUFACTURER', 'PRODUCT MODEL', 'PRODUCT CODE', 'SHORT DESC', 'FULL DESC', 'REVIEWS NUM', 'PRICE', 'DISCOUNT PRICE', 'AVAILABILITY', 'DESCRIPTION', 'ROBOTS', 'SEO TEXT', 'VISITS', 'DATE ADD',], $custom_head); 
            fputcsv($output, $head);
            foreach($products_info as $uri => $info){
                $row = [
                    html_entity_decode($info['url']),
                    html_entity_decode($info['h1']),
                    html_entity_decode($info['category_name']),
                    html_entity_decode($info['parent_category_name']),
                    html_entity_decode($info['breadcrumbs']),
                    html_entity_decode($info['product_manufacturer']),
                    html_entity_decode($info['product_model']),
                    html_entity_decode($info['product_code']),
                    html_entity_decode($info['short_desc']),
                    html_entity_decode($info['full_desc']),
                    html_entity_decode($info['reviews_num']),
                    html_entity_decode($info['price']),
                    html_entity_decode($info['discount_price']),
                    html_entity_decode($info['availability']),
                    html_entity_decode($info['description']),
                    html_entity_decode($info['robots']),
                    html_entity_decode($info['seo_text']),
                    html_entity_decode($info['visits']),
                    html_entity_decode($info['date_add']),
                ];
                foreach($custom_head as $col){
                    if (isset($info['attrs'][$col])){
                        $row[] = implode(' / ', $info['attrs'][$col]);
                    } else {
                        $row[] = '';
                    }
                }
            ...
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_info_name'])){
                $q = $db->mysql_query("TRUNCATE table ".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_name']);
                if (!$q){
                    exit('Error while truncating');
                }
                $q = $db->mysql_query("TRUNCATE table ".$GLOBALS['SEOSHIELD_CONFIG']['track_products_visits']['table_info_name']);
                if (!$q){
                    exit('Error while truncating');
                }
                exit('Done');
                ...
                switch ($data->destination) {
                    ...
                    case 'static_meta:by_id':
                        ...
                        if ($save_data_from_shield) {
                            $__file = fopen($cache_file, 'w');
							$lock = flock($__file, LOCK_EX);
							$bytes_written = file_put_contents($cache_file, '<?php return ' . var_export($cache_new_data, true) . ';');
							flock($__file, LOCK_UN);
							fclose($__file);
