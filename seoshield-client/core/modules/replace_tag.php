...
{
    public function html_out_buffer_alter($out_html)
    {
    ...
    {
        if (!empty($GLOBALS['SEOSHIELD_CONFIG']['replace_tag']) && !empty($GLOBALS['SEOSHIELD_CONFIG']['replace_tag_page_types'])) {
            ...
            if ($is_target_page || $GLOBALS['SEOSHIELD_CONFIG']['page_uri'] == '/'){
                foreach ($GLOBALS['SEOSHIELD_CONFIG']['replace_tag'] as $tag) {
                    ...
                    $out_html = preg_replace_callback('#(<(?:(?!\/)'.$tag.')(?:(?:\s+(?:[\w-]+)(?:\s*=\s*(?:"(?:[^"]*)"|\'(?:[^\']*)\'|(?:[^\'">\s]+)))?)+\s*|\s*)(?:\/)?>)#si', function ($finder_outer) use ($tag, &$out_html) {
                        ...
                        if (isset($finder_inner[0]) && !empty($finder_inner[0])) {
                            ...
                            $clss = preg_replace(['#("\s+)#', '#\s+#'], ['"', ' '], $clss_a);
                            if ('' == $clss) {
                                $clss = 'class="stag'.$tag.'"';
                            }
                        }

                        $result = '<span '.implode(' ', [$attrs, $clss]).'>';
