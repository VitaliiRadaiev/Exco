...
class SeoShieldModule_api_generate_content extends seoShieldModule
{
    ...
    {
        $GLOBALS['SEOSHIELD_DATA']['api_generate_content'] = [];
        $GLOBALS['SEOSHIELD_DATA']['init_api_generate_content'] = [];
        $GLOBALS['SEOSHIELD_DATA']['api_generate_content_options'] = [];
    ...
    public function html_out_buffer($out_html)
    {
        ...
        if (sizeof($GLOBALS['SEOSHIELD_DATA']['init_api_generate_content']) > 0) {
            ...
            if (false == $debug_generate_content) {
        ...
        foreach ($contents_res as $content_hash => $content_data) {
            if (empty($content_data['text']) || (isset($GLOBALS['SEOSHIELD_DATA']['static_meta']['content']) && !empty($GLOBALS['SEOSHIELD_DATA']['static_meta']['content'])
                    && isset($content_data['hide_template']) && true == $content_data['hide_template'])) {
                continue;
            }

            $template_res = ['before' => '', 'after' => ''];
            ...
            if (false !== strpos($out_html, '<!--{ss_api_content_start:'.$content_hash.':ss_api_content_start}-->')) {
                if (false !== strpos($out_html, '<!--{ss_api_content_end:'.$content_hash.':ss_api_content_end}-->')) {
