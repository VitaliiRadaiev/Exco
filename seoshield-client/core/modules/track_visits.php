...
class SeoShieldModule_track_visits extends seoShieldModule
{
    private function check_and_create_table()
    {
        return $GLOBALS['SEOSHIELD_CONFIG']['mysql']->mysql_query('CREATE TABLE IF NOT EXISTS `'.$GLOBALS['SEOSHIELD_CONFIG']['track_visits']['table_name']."` (
            ...
            `filter_3` varchar(255) NOT NULL COLLATE utf8_general_ci,
            `filter_4` varchar(255) NOT NULL COLLATE utf8_general_ci,
    ...
    private function get_filter_info($out_html, $position = 1)
    {
    ...
    private function get_filters_info($out_html)
    {
        $filters_info = array();
        if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['selected_filters_selector'])) {
            preg_match_all($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['selected_filters_selector'], $out_html, $filters_matches);
            if (isset($filters_matches['filter_name']) && isset($filters_matches['filter_value'])) {
                foreach ($filters_matches['filter_name'] as $index => $filter_name) {
                    $filters_info[] = $filter_name.': '.$filters_matches['filter_value'][$index];
                }
            }
        }
        $filters_info = array_slice($filters_info, 0, 4);
        sort($filters_info);
        return $filters_info;
    }
    ...
    private function get_category_name($out_html)
    {
        ...
        $category_name = preg_replace('/[\x{10000}-\x{10FFFF}]/u', '', $category_name);
    ...
    public function html_out_buffer($out_html)
    {
        ...
        if (!isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits'])) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: empty track_visits module config [track_visits]--></body>', $out_html);
            }
        ...
        if (!isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['selectors'])
            ...
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: empty fields in track_visits module config [track_visits]--></body>', $out_html);
            }
        ...
        if (!$GLOBALS['SEOSHIELD_CONFIG']['mysql'] instanceof seoShieldDb) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: problems in DB config [track_visits]--></body>', $out_html);
            }
        ...
        if (!$this->check_and_create_table()) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: table in DB was not created [track_visits]--></body>', $out_html);
            }
        ...
        if (!$matched) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: selectors not matched [track_visits]--></body>', $out_html);
            }
        ...
        if (false === strpos($out_html, $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['product_selector'])) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: products not matched [track_visits]--></body>', $out_html);
            }
        ...
        if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['filter_uri_part']) && false === strpos($GLOBALS['SEOSHIELD_CONFIG']['page_url_alt'], $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['filter_uri_part'])) {
            if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
                $out_html = str_replace('</body>', '<!--SEOSHIELD_WARNING: URI not matches [track_visits]--></body>', $out_html);
            }
        ...
        $filters_info = $this->get_filters_info($out_html);
        foreach (range(1,4) as $index) {
            $row['filter_'.$index] = '';
            if (isset($filters_info[$index-1])){
                $row['filter_'.$index] = $filters_info[$index-1];
            }
        }
        //if (isset($GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) && $GLOBALS['SEOSHIELD_CONFIG']['track_visits']['debug']) {
