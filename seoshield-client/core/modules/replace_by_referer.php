...
{
    public function check_substring($needle, $haystack)
    {
        if (2 == substr_count($needle, '#')) {
            $pattern = $needle;
            ...
            if (false !== strpos($needle, $haystack)) {
                return true;
            } else {
                return false;
            }
    ...
    public function detect_user_source()
    {
        $_COOKIE = [];
        $source = ['', '', ''];
        ...
        if (!isset($_COOKIE['ss_source'])
            ...
            $source = [$referer, $traffic, $country];
    ...
    public function html_out_buffer($out_html)
    {
        ...
        if (file_exists($csv_file_name)
            ...
            foreach ($file_res as $cols) {
                ...
                if (false !== strpos($referer, $referer_csv)
                    && false !== strpos($traffic, $traffic_csv)
                    && false !== strpos($country, $country_csv)
