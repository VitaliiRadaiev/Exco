...
class SeoShieldModule_pages_adverts extends seoShieldModule
{
    ...
    public function check_cache_file($file_key)
    {
    ...
    {
        if (!isset($GLOBALS['SEOSHIELD_CONFIG']['pages_adverts_config']) || 'GET' !== $_SERVER['REQUEST_METHOD']) {
            return $out_html;
        }

        if (isset($GLOBALS['SEOSHIELD_CONFIG']['pages_adverts_config']['hide_if_page_content'])
            && true === $GLOBALS['SEOSHIELD_CONFIG']['pages_adverts_config']['hide_if_page_content']
            && isset($GLOBALS['SEOSHIELD_DATA']['static_meta_replace']['content'])
            && true === $GLOBALS['SEOSHIELD_DATA']['static_meta_replace']['content']
            && isset($GLOBALS['SEOSHIELD_CONFIG']['is_static_page']) && true !== $GLOBALS['SEOSHIELD_CONFIG']['is_static_page']) {
            return $out_html;
        }
        ...
        if (false !== strpos($out_html, $config['pages_adverts'])) {
            $config['replace'] = 'pages_adverts';
        } elseif (false !== strpos($out_html, $config['pages_keywords'])) {
            $config['replace'] = 'pages_keywords';
        }

        if (false === $config['replace'] && true !== $config['debug']) {
            return $out_html;
        }
        ...
        if (!file_exists($this->cache_files['page_cache']) || $config['debug']) {
            ...
            if (false == $cache['pages_adverts_all'] || 0 == sizeof($cache['pages_adverts_all'])) {
                return $out_html;
            }
            ...
            if (!is_array($cache['pages_adverts_f']['adverts']) || 0 == sizeof($cache['pages_adverts_f']['adverts'])
                || $cache['pages_adverts_time'] > $cache['pages_adverts_config']['pages_adverts_time']) {
                $cache['pages_adverts'] = [];
                $cache['pages_keywords'] = [];
                ...
                $cache['pages_adverts_f'] = [];
                $cache['pages_adverts_f']['adverts'] = [];
                ...
                do {
                    ...
                    foreach ($cache['pages_adverts_copy'] as $adv_url => $advs_res) {
                        ...
                        if (0 == sizeof($cache['pages_adverts_copy'][$adv_url])) {
            ...
            $cache['page_cache'] = [];
            if ('pages_adverts' == $config['replace'] && sizeof($cache['pages_adverts_f']['adverts']) > 0) {
            ...
            } elseif ('pages_keywords' == $config['replace'] && sizeof($cache['pages_adverts_f']['keywords']) > 0) {
                do {
        ...
        if (file_exists($this->cache_files['page_cache']) || $config['debug']) {
            ...
            if (isset($cache['page_cache']) && sizeof($cache['page_cache']) > 0) {
                ...
                foreach ($cache['page_cache'] as $adv_res) {
                    if ('pages_keywords' == $config['replace'] && !isset($adv_res['keyword'])) {
                        continue;
                    }

                    $page_adverts_txt .= '<li>';
                    if ('pages_adverts' == $config['replace']) {
                ...
                if (isset($config['add_css']) && 'string' == gettype($config['add_css'])) {
