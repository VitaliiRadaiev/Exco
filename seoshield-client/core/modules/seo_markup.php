    ...
    public function html_out_buffer($out_html)
    {
        ...
        if (isset($GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_product_page']) && isset($GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_listing_page'])) {
            if ('' !== $GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_product_page'] && '' !== $GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_listing_page']) {
                if (false !== strpos($out_html, $GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_product_page']) || false !== strpos($out_html, $GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_listing_page'])) {
                    if (1 == preg_match('#<h1[^>]*?>(.*?)</h1>#s', $out_html, $page_h1)) {
                        $page_components['h1'] = $page_h1[1];
                    }
                    if (1 == preg_match('#<meta[^>]*?description[^>]*?content=["\'](.*?)["\'][^>]*?>#s', $out_html, $page_description)) {
                        $page_components['description'] = $page_description[1];
                    }

                    if (1 == preg_match($GLOBALS['SEOSHIELD_CONFIG']['seo_markup']['content_markup_img'], $out_html, $page_image)) {
