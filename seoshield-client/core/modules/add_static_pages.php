    ...
    public function start_cms()
    {
        if (file_exists(SEOSHIELD_ROOT_PATH.'/data/static_meta.cache.php') && file_exists(SEOSHIELD_ROOT_PATH.'/data/templates.cache.php')) {
            ...
            if (!is_null($current_url)
                ...
                && (
                    'static_page' == $static_data[$current_url]['type'] ||
                    'static_category' == $static_data[$current_url]['type']
                )
                ) {
                ...
                if (!is_null($content)) {
                    ...
                    if ('static_category' == $current_data['type'] && isset($current_data['category']['_id'])) {
                        $subcategories = [];
                        $child_pages = [];
                        foreach ($static_data as $url => $data) {
                            ...
                            if (isset($data['type'])) {
                                if (
                                    ...
                                    if ('static_category' == $data['type']) {
                                        $subcategories[] = '<a href="'.$url.'">'.$data[2].'</a>';
                                    } elseif ('static_page' == $data['type']) {
